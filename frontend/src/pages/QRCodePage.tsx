...
export default function QRCodePage({ refreshStatus }: Props) {
	...
	// Generate QR as a data URL (local generation)
	useEffect(() => {
		if (!trimmed) {
			setQrDataUrl('')
			setQrError(null)
	...
	useEffect(() => {
		...
		if (!restore || restore.kind !== 'qr') return
		if (restore.job_id && lastRestoreJobIdRef.current === restore.job_id) return
		const nextText = String(restore?.payload?.text ?? '')
		setText(nextText)
